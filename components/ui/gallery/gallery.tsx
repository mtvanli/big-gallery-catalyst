...
const GalleryContent = forwardRef<ElementRef<'figure'>, ComponentPropsWithRef<'figure'>>(
  ({ className, children, ...props }, ref) => {
    return (
      <figure
        className={cn('relative aspect-square h-full max-h-[848px] w-full', className)}
        ref={ref}
        {...props}
      >
        {children}
  ...
  ({ children, className, ...props }, ref) => {
    return (
      <nav
        ...
        className={cn(
          '-mx-1 mt-3 flex w-full flex-nowrap items-center gap-4 overflow-x-auto px-1 py-1 md:mt-12 md:gap-6 md:ml-20',
          className,
        )}
        ref={ref}
        {...props}
  ...
  ({ className, children, imageIndex, onClick, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn(
          'inline-block h-24 w-24 flex-shrink-0 flex-grow-0 focus:outline-none     ',
          className,
        )}
        onClick={(e) => {
