...
const ProductDetails = ({ product }: { product: NonNullable<Product> }) => {
  ...
  return (
    <div>
      ...
      <h1 className="mb-4 text-4xl font-black lg:text-4xl">{product.name}</h1>
       {
            product.customFields.map((customField) => (
              <div key={customField.entityId} className='pl-2 pt-3.5'>
                {customField.name == 'Store'? <Link href={customField.value} target="_blank"> <OpenInNewIcon /> </Link>: null } 
  ...
  return (
    <div className="lg:col-span-2">
      {Boolean(product.description) && (
        <>
          <h2 className="mb-4 text-xl font-bold md:text-2xl">Highlights</h2>
...
export default async function Product({ params, searchParams }: ProductPageProps) {
  ...
  return (
    <>
      ...
      <div className="mb-20 mt-20 lg:grid lg:grid-cols-2 lg:grid-rows-1 lg:gap-8 grid-flow-col">
